...
#####Figure 5: Protein PRS Associations with AD
...
ggplot(data = prot_prs_meta_min_p) + geom_col(mapping = aes(x=Protein, y=P_MinusLog10)) + geom_hline(yintercept=1.3, linetype="dashed", color = "red") + expand_limits(y = c(0, 2.5)) + theme(axis.text.x=element_text(angle = -90, hjust = 0)) + geom_text(aes(x=Protein, y=P_MinusLog10,label=Threshold, vjust=-0.5))
...
#Sort labels
ggplot(data = prot_prs_ind) + geom_col(mapping = aes(x=Protein_Sample, y=P_MinusLog10, fill=Sample)) + geom_hline(yintercept=1.3, linetype="dashed", color = "red") + theme(axis.text.x=element_text(angle = -90, hjust = 0))
