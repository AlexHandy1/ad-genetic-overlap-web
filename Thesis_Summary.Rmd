...
ggplot(data = prot_prs_meta_min_p) + geom_col(mapping = aes(x=Protein, y=P_MinusLog10)) + geom_hline(yintercept=1.3, linetype="dashed", color = "red") + theme(axis.text.x=element_text(angle = -90, hjust = 0))
...
prot_prs_ind <- prot_prs_ind %>% mutate(PrtSam = paste(Protein, Sample))

#Sort labels
ggplot(data = prot_prs_ind) + geom_col(mapping = aes(x=PrtSam, y=P_MinusLog10)) + geom_hline(yintercept=1.3, linetype="dashed", color = "red") + theme(axis.text.x=element_text(angle = -90, hjust = 0))
